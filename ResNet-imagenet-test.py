...
imageWidth=224
num_classes = 5
num_steps = int(300000/batch_size)
...
#val_dataset = val_dataset.repeat(epoch*600)
...
if __name__ == '__main__':
    ...
    #vX = tf.placeholder(tf.float32, shape=[None, 256, 256, 3], name='TX')
    #vY = tf.placeholder(tf.int64, shape=[None], name='labels')
    is_training = tf.placeholder(tf.bool, name='IsTraining')
    
    global_step = tf.Variable(0, trainable=False)
    boundaries = [100, 500, 1000, 1500, 3000, 5000]
    values = [0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001]
    learning_rate = tf.train.piecewise_constant(global_step, boundaries, values)
    
    #X = X - [123.68, 116.78, 103.94]
    
    #distorted_images = tf.random_crop(X, size=[batch_size, imageHeight, imageHeight, 3])
    distorted_images = tf.image.resize_images(X, [224,224])
    distorted_images = tf.image.per_image_standardization(distorted_images)
    #distorted_images=distorted_images-[123.68, 116.78, 103.94]
    ...
    with tf.Session(config=cfg) as sess:
        ...
        try:
            for i in range(0,num_steps):
                ...
                train_dict = {X: input_image, Y: groundtruth_lists,is_training: False}
                ...
                vloss_, vacc_ = sess.run([loss, accuracy], feed_dict=val_dict)
                totalacc+=vacc_
                avaacc=totalacc/(i+1)*1.0
                """
                #train_text = '[ Train ] Step: {}, Loss: {:.4f}, Accuracy: {:.4f}, AveAccuracy: {:.4f},learning_rate: {:.4f}'.format(i, loss_, acc_,avaacc0,lr)
                train_text = 'Step: {},[Train] Loss:{:.4f}, Acc: {:.4f}, AveAcc: {:.4f} '.format(i, loss_, acc_,avaacc0)
                #[Val] Loss:{:.4f}, Acc: {:.4f}, AveAcc: {:.4f} ,vloss_, vacc_,avaacc
                print(train_text)
